...
public class CorridorBase<T extends R_Node> implements Iterable<T> {
	...
	public void addNode(T r_node) {
                System.out.println("\tadd node "+r_node.getName()+" "+isValid(r_node));
	...
	private void calculateNodeMilePoints() {
                //System.out.println("calculate node mile points "+getName()+" "+r_nodes);
		...
		for (T n: r_nodes) {
			if (previous != null) {
				Distance m = nodeDistance(previous, n);
                                ...
                                //System.out.println("\t distances is "+m);
				if (m == null)
					continue;
